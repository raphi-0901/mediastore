...
class OrderMade extends Mailable
{
    ...
    public function build()
    {
        ...
        $mjtable = '<mj-table><tr style="border-bottom:2px solid #eef2f7;text-align:left;padding:15px 0;" ><th style="padding: 0 15px 0 0;">Name</th></tr>';

        foreach ($this->order->devices as $device)
            $mjtable .= '<tr><td style="padding: 0 15px 0 0;">' . $device->name . '</td></tr>';
    ...
    <mj-body background-color="#fafbfe" width="600px">
        <mj-wrapper background-color="#fff" padding="0" css-class="body-section">
            <mj-section full-width="full-width" padding="0">
                <mj-column mj-class="bg-success" border-bottom="10px solid #42d29d" width="100%" padding="20px">
                   <mj-image width="30px"
            ...
            <mj-section mj-class="text-container">
                <mj-column>
                    <mj-text mj-class="h1" align="center">Vielen Dank für Ihre Bestellung</mj-text>
                    <mj-divider border-color="#eef2f7"></mj-divider>
                    <mj-text align="center">Die Bestellung wurde in unser System aufgenommen.</mj-text>
                    <mj-text mj-class="text-success" font-size="22px" font-weight="bold" align="center">' . $this->order->from->isoFormat("Do MMM. YYYY") . ' bis ' . $this->order->to->isoFormat("Do MMM. YYYY") . '</mj-text>
                    <mj-text align="center">Bitte denken Sie daran, die Geräte fristgerecht zurückzugeben!</mj-text>
                    <mj-divider border-color="#eef2f7" padding="20px"></mj-divider>
                    <mj-text mj-class="h2">Bestellübersicht</mj-text>' . $mjtable .
            '</mj-column>
            </mj-section>
        ...
        return $this->html($html)
            ->subject("Ihre Bestellung: " . $this->order->created_at->isoFormat("Do MMM. YYYY"));
    }
}
